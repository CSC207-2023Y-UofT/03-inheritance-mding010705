...
public abstract class Bag {
     ...
     *       and false otherwise.
     */
    public boolean addItem(String itm){
        if (this.numberOfContents < this.capacity){
            this.numberOfContents++;
            String[] oldCont = this.contents.clone();
            this.contents = new String[this.numberOfContents];
            for (int i = 0; i < this.numberOfContents; i++){
                if (i == this.numberOfContents - 1) {
                    this.contents[i] = itm;
                } else {
                    this.contents[i] = oldCont[i];
                }
            }
            return true;
        }
    ...
    public String popItem(){
        ...
        this.numberOfContents--;
        String[] tmpCont = this.contents.clone();
        this.contents = new String[this.numberOfContents];
        for(int i = 0; i < this.numberOfContents; i++){
            this.contents[i] = tmpCont[i];
        }
        return tmpCont[this.numberOfContents];
    }
